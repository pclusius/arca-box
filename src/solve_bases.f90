...
contains
  ...
  contains
    ...
    real(dp) function JACDC(test)
      ...
      IF (UCASE(Fill_formation_with) /= 'DMA') THEN
        i = i+1
        CALL get_acdc_J(SA,BASE,dummy1,CS,T,IPR,GTIME,.true.,testJ,dummy2,dummy3,GTIME%dt, .false.)
        JACDC = testJ
      END IF
      IF (UCASE(Fill_formation_with) /= 'NH3') THEN
        if (DMA_f > 0 .or. UCASE(Fill_formation_with) == 'DMA') THEN
          ...
          IF (UCASE(Fill_formation_with) == 'DMA') THEN
            ...
            JACDC = 0d0
          END IF
          CALL get_acdc_D(SA,BASE,dummy1,CS,T,GTIME,.true.,testJ,dummy2,GTIME%dt, .false.)
          JACDC = JACDC + testJ
