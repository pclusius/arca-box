...
subroutine acdc_driver(neqn,nclust,nout_all,c,cs_ref,temperature,ipr,t_max,t_iter,solve_ss,use_solver,ipar,ok,j_out)
	...
	real(kind(1.d0)) :: c00(neqn), ci(neqn), c_1(neqn), ci_temp(neqn), f_out(neqn), f_out_mp(neqn), eps_max	! temporary c and dc/dt
	real(kind(1.d0)) :: c_out(n_charges)				! total concentration of outgrown particles at the beginning
	integer, parameter :: nt = 3, nt_ss = 12			! number of timeranges for integration by the solver
	...
	real(kind(1.d0)) :: parameters(1+n_charges)			! one or more of the following parameters (max. 4 param.):
	...
	if (use_solver) then
		...
		if (solve_ss) then
            t_ss = (/t00+(/1.d-4, 1.d-2, 1.d0, 1.d2, 1.d3, 2.d4, 3.d4, 4.d4, 5.d4, 1.d5, 1.d6, 1.d7/)/)
            nt_tot = nt_ss
        else
            t = (/t00+(/1.d-5, 1.d-3/), t_max/)
            nt_tot = nt
        end if
	...
	if (solve_ss) then
		...
		j_out = c(nout_all)/(t0-t00)
		! Add the initial out-of-system population back to c
		c(nout_all) = c_out+c(nout_all)
	end	if
