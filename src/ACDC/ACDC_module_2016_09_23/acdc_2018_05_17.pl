...
if($help){
		...
		\t\tNote: use this option only for a big box-shaped system - no fancy boundary conditions are possible\n
		\t--loop_cs
		...
		\t--wall_terms
		\t--wl
		\t\tavailable wall loss parameterizations: cloud4_JA, cloud4_JK, cloud4_AK, cloud4_simple, cloud3, ift, diffusion, value in 1/s\n
		\t--flowtube_radius
...
if(($use_coag_sinks+$use_wall_terms+$use_dilution+$variable_cs+$l_wl_mcmc)>0 || $coag_terms_in ne '' || $wall_terms_in ne '' || $#wl_only>=0 || $#wl_mcmc_only>=0){
	...
	if($use_coag_sinks==0 && $variable_cs==0 && $coag_terms_in eq ''){
		...
		print "Not using coagulation sinks.\n";
	}else{
...
if($lfortran){
	...
	print SYST "module acdc_systemD\n\nimplicit none\n\n";
	if(!$lloop){
		...
		if($lloop_restrict){
			print SYST "logical, parameter :: restrict_system = .true.				! restrict the set of clusters\n";
		}else{
			print SYST "logical, parameter :: restrict_system = .false.				! restrict the set of clusters\n";
		}
	}
	if($l_jlim){
...
if($lfortran){
	...
	if($lloop && !$lloop_cs && !$l_save_outgoing){
		print OUT "subroutine fevalD$append_to_subroutine_names(neqn,t,c,f_out,coef,ipar)\n";
	}else{
		print OUT "subroutine fevalD$append_to_subroutine_names(neqn,t,c,f,coef,ipar)\n";
	}
	if($lloop){
		if($nmol_types_used>1){
			print OUT "use acdc_systemD, only : monomer_indices\n";
		}
		print OUT "use monomer_settingsD, only : sources_and_constants\n";
		if($l_j_in){
	...
	if($lloop){
		print OUT "\tlogical, save :: isconst(nclust) = .false.\n";
		...
		print OUT ", source(nclust)\n";
		print OUT "\tinteger, save :: fitted(nclust,0:nclust)=0";
		if($nmol_types_used>1){
			print OUT ", nclust_inc, indices(nclust,$nmol_types_used)";
		...
		print OUT "\tinteger, save :: ind_lin_loss($max_n_flux,0:$ind_lin_loss[0])=0,ind_lin_form($max_n_flux,0:$ind_lin_form[0])=0,fitted($max_cluster_number,0:$max_cluster_number)=0\n";
		if($l_reac_in_clus){
	...
	if($lloop){
		...
		if($l_mcmc_any){
			print OUT "\t\tcall get_collD$append_to_subroutine_names(K,coef)\n";
		}elsif( $variable_temp){
			print OUT "\t\tcall get_collD$append_to_subroutine_names(K,coef(1))\n";
		}else{
			print OUT "\t\tcall get_collD$append_to_subroutine_names(K)\n";
		}
		...
		if(!$disable_evap){
			if($l_mcmc_any){
				print OUT "\t\tcall get_evapD$append_to_subroutine_names(E,coef)\n";
			}elsif( $variable_temp){
				print OUT "\t\tcall get_evapD$append_to_subroutine_names(E,K,coef(1))\n";
			}else{
				print OUT "\t\tcall get_evapD$append_to_subroutine_names(E)\n";
			}
		}
		if($l_use_get_losses){
			print OUT "\t\tcall get_lossesD$append_to_subroutine_names(loss)\n";
		}
		if($l_j_in){
		...
		print OUT "\t\tcall initialize_parametersD$append_to_subroutine_names(coef_quad,coef_lin,ind_quad_loss,ind_quad_form,&\n\t\t&\tind_lin_loss,ind_lin_form,ind_quad_loss_extra,ind_quad_form_extra,&\n\t\t&\tsource,isconst,fitted";
	...
	if($lloop){
		...
		$lines{'l_evap'} = "if (";
		if($nmol_types_used>1){
			if($disable_nonmonomer_evaps && !$disable_nonmonomers){
				$lines{'l_evap'} .= "min(sum(indices(i,:)),sum(indices(j,:))).eq.1 .and. ";
			}
			if($lloop_cs){
				$lines{'l_evap'} .= ".not.all(ij_ind.eq.ij_ind_cs)";
			}else{
				$lines{'l_evap'} .= "clust_from_indices($nmol_ranges_ij_ind).gt.0";
			}
		}else{
			if($disable_nonmonomer_evaps && !$disable_nonmonomers){
	...
	print OUT "\t\tend do\n";
	print OUT "\tend do\n";
	print OUT "\nend subroutine fevalD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
	...
	print OUT "subroutine jevalD$append_to_subroutine_names(neqn,t,c,ml,mu,dfdc,ldim,coef,ipar)\n";
	...
	print OUT ",ml,mu\n";
	print OUT "\nend subroutine jevalD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran){
	print OUT "subroutine formationD$append_to_subroutine_names(neqn,c";
	if($lloop){
		print OUT ",j_out";
		if($l_jlim){
			print OUT ",j_lim";
	...
	if($lloop){
		...
		if($l_jlim && !$disable_evap){
			print OUT "\treal(kind(1.d0)), save :: E(nclust,";
			if($disable_nonmonomer_evaps && $nmol_types_used==1){
				print OUT "1)";
			}else{
				print OUT "nclust)";
		...
		if($nmol_types_used>1){
			print OUT "\tinteger, save :: nclust_inc, indices(nclust,$nmol_types_used)";
			print OUT ", clust_from_indices($nmol_comp_list)";
			if($disable_nonmonomers){
				print OUT ", n_monomers($nmol_types_used)";
			}
		...
		print OUT "\tinteger, save :: ind_quad_loss_extra($max_n_flux,0:$ind_quad_loss_extra[0])=0,ind_quad_form_extra($max_n_flux,0:$ind_quad_form_extra[0])=0\n";
		print OUT "\tinteger, save :: ind_lin_loss($max_n_flux,0:$ind_lin_loss[0])=0,ind_lin_form($max_n_flux,0:$ind_lin_form[0])=0,fitted(nclust,0:nclust)=0\n";
		print OUT "\tlogical, save :: isconst($size_C_out)=.false.\n\n";
	}
	if($l_jlim){
	...
	if($lloop){
		...
		if($nmol_types_used>1 || $l_jlim || !$disable_nonmonomers || $lgrowth_for_j_tests){
			...
			if($l_jlim){
				...
				if(!$disable_evap){
					if($lloop_cs){
						# Change the condition for evaporation a bit, as outgrown clusters are not placed in cs_clust
						# in the formation subroutine
						$lines{'l_evap'} =~ s/\.not\.all\(ij_ind\.eq\.ij_ind_cs\)/clust_from_indices\($nmol_ranges_ij_ind\)\.gt\.0/;
					}
					print OUT "\t\t\t\t\t$lines{'l_evap'}";
					if($nmol_types_used>1){
						print OUT "\t\t\t\t\t\tij = clust_from_indices($nmol_ranges_ij_ind)\n";
	...
	print OUT "\nend subroutine formationD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran && !$lloop){
	print OUT "subroutine initialize_parametersD$append_to_subroutine_names(coef_quad,coef_lin,ind_quad_loss,ind_quad_form,&\n\t\t\t&ind_lin_loss,ind_lin_form,ind_quad_loss_extra,ind_quad_form_extra,&\n\t\t&\tsource,isconst,fitted";
	if($l_mcmc_any || $variable_temp || $variable_cs || $variable_ion_source){
		print OUT ",coef";
	}
	print OUT ",ipar)\n";
	...
	print OUT "\n\tcall get_rate_coefsD$append_to_subroutine_names(coef_quad,coef_lin";
	if($l_mcmc_any || $variable_temp || $variable_cs || $variable_ion_source){
		print OUT ",coef";
	}
	print OUT ")\n";
	print OUT "\nend subroutine initialize_parametersD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
	...
	print OUT "subroutine get_rate_coefsD$append_to_subroutine_names(coef_quad,coef_lin";
	if($l_mcmc_any || $variable_temp || $variable_cs || $variable_ion_source){
		print OUT ",coef";
	}
	print OUT ")\n";
	...
	if($l_use_get_losses){
		...
		print OUT "\tcall get_lossesD$append_to_subroutine_names($line)\n";
	}
	if($variable_cs){
		if($one_cs_for_all){
	...
	print OUT "\nend subroutine get_rate_coefsD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran){
	if($l_use_get_losses){
		...
		if(!$lloop){
			print OUT "subroutine get_lossesD$append_to_subroutine_names($lines{'inp'})\n";
			print OUT "\timplicit none\n\treal(kind(1.d0)) :: $lines{'def'}\n\n";
		}else{
			print OUT "subroutine get_lossesD$append_to_subroutine_names(loss)\n";
			if(!$disable_coag_sinks && $coag_terms =~ m/^bg_loss$/i){
...
if($lfortran && $l_use_get_losses){
	...
	print OUT "\nend subroutine get_lossesD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
}
...
if($lfortran){
	...
	print OUT "subroutine get_collD$append_to_subroutine_names(K";
	if($l_mcmc_any){
		print OUT ",coef";
	}elsif($variable_temp){
...
if($lfortran){
	print OUT "\nend subroutine get_collD$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
}else{
	if($variable_temp){
		print COLL "\nend";
	}
...
if(!$disable_evap){
	...
	if($lfortran){
		print OUT "subroutine get_evapD$append_to_subroutine_names(E";
		if($l_mcmc_any){
			print OUT ",coef";
		}elsif($variable_temp){
...
################################################################################
	...
	if($lloop){
		...
		print OUT "\n";
		print OUT "\treal(kind(1.d0)) :: E(nclust,";
		if($disable_nonmonomer_evaps && $nmol_types_used==1){
			print OUT "1)";
		}else{
			print OUT "nclust)";
		}
		print OUT ", K(nclust,nclust), temperature\n";
		if($nmol_types_used>1){
			print OUT "\tinteger :: nclust_inc, indices(nclust,$nmol_types_used)";
			print OUT ", clust_from_indices($nmol_comp_list)";
			print OUT "\n";
		}
		print OUT "\tinteger :: ij_ind($nmol_types_used), i, j, ij, imol\n";
...
################################################################################
			...
			print OUT "\tinteger :: n_monomers($nmol_types_used)\n";
...
################################################################################
	...
	if($lfortran){
		print OUT "\nend subroutine get_evapD$append_to_subroutine_names\n";
	}
}else{
	if(!$lfortran){
