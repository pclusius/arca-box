module driver_acdc_J_0x4
use solution_settings_new
use acdc_system_0x4, only : small_set_mode, n_charges		! simulation mode and numbers of charging states
use acdc_system_0x4, only : n_variables, variable_temp		! information on parameters that vary based on input
use acdc_simulation_setup_0x4, only : solve_ss, j_flux, use_solver	! logicals for the solution approaches
use acdc_simulation_setup_0x4, only : sources_and_constants ! for determining fitted concentrations
use acdc_equations_0x4, feval=>feval_0x4, jeval=>jeval_0x4, formation=>formation_0x4

implicit none

contains

subroutine acdc_driver_0x4(neqn,nclust,nout_all,c,cs_ref,temperature,ipr,t_max,t_tot,t_iter,ipar,ok,j_out)
	...
	if (solve_ss .or. j_flux) then
		...
		if (small_set_mode) then
            ...
            continue
			! call large_formation(neqn,c,j_out(1),ipar,parameters)
		end if
	else
...
end subroutine acdc_driver_0x4
