...
use acdc_simulation_setup_0x5, only : get_system_size, get_vapor_indices                ! parameters related to the simulation system size
use acdc_simulation_setup_0x5, only : group_size_bins, nbins
use acdc_simulation_setup_0x5, only : solve_ss, use_solver                              ! logicals for the solution approaches
use acdc_system_0x5, only : small_set_mode, n_neutral_monomers, neutral_monomers,n_charges, nclust_max,neq,cluster_names,n_A_in_clusters   ! simulation mode and numbers of vapors and charging states
use driver_acdc_J_0x5, only : acdc_driver_0x5
    ...
    call acdc_driver_0x5(neq_syst,nclust_syst,nout_syst,c,cs_ref,temp,ipr,t_in,t_tot,t_iter,ipar,int_ok,j_out)
    ! j_acdc = sum(j_out)
    ! j_acdc = sum(j_out)
    j_acdc = -1d0
    j_acdc(1) = sum(j_out)
    ...
    if (printnow) THEN
        print *, '---- Cluster population from ACDC_05 ----'
