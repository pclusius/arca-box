...
SUBROUTINE Condensation_apc(vapour_prop, conc_vap, dmass, dt_cond, d_dpar,d_vap)
  ...
  REAL(dp), DIMENSION(n_bins_particle,n_cond_tot)   :: Kelvin_Effect
  REAL(dp), DIMENSION(n_bins_particle,n_cond_tot)   :: kohler_effect
  ...
  REAL(dp)                                          :: n_conc(n_bins_particle)
  REAL(dp)                                          :: diameter(n_bins_particle)
  REAL(dp)                                          :: volume(n_bins_particle)
  REAL(dp)                                          :: mass(n_bins_particle) ! [kg/m^3]
...
SUBROUTINE Coagulation_routine(dconc_coag, dt_coag, d_npar) ! Add more variables if you need it
    ...
    REAL(dp), DIMENSION(n_bins_particle,n_bins_particle)  :: sticking_prob           ! alpha in Fuchs' beta, S&P (2016) p. 550, eq. 13.56
    REAL(dp), DIMENSION(n_bins_particle)                  :: slip_correction
    REAL(dp), DIMENSION(n_bins_particle)                  :: diffusivity,dist
    REAL(dp), DIMENSION(n_bins_particle)                  :: speed_p
    REAL(dp), DIMENSION(n_bins_particle)                  :: free_path_p
...
do j =1, n_bins_particle
  do m = j, n_bins_particle
    ...
    ! IF (n_conc(j) > 1.d0 .and. n_conc(m) > 1.d0) dconc_coag(j,m) = a * (1-exp(-coagulation_coef(j,m) * dt_coag)) * n_conc(j) * n_conc(m) ! 1/m^3
    IF (n_conc(j) > 1.d0 .and. n_conc(m) > 1.d0) dconc_coag(j,m) = a * coagulation_coef(j,m) * n_conc(j) * n_conc(m) ! 1/m^3s
    if ((dconc_coag(j,m)>n_conc(j)) .or. (dconc_coag(j,m)>n_conc(m))) print*, 'j,m, dc(j,m), c(j), c(m)', j, m, dconc_coag(j,m), n_conc(j), n_conc(m)
...
function collision_rate(jj,diameter, mass,vapour_prop)
    ...
    REAL(dp), DIMENSION(n_bins_particle) :: collision_rate          ! Collision rate of gas jj and particles
    REAL(dp), DIMENSION(n_bins_particle) :: knudsen                 ! Knudsen number []
    REAL(dp), DIMENSION(n_bins_particle) :: Diff_par                ! Particle diffusion coefficient [m^2 s^-1]
    REAL(dp), DIMENSION(n_bins_particle) :: slip_correction         ! Cunningham slip correction []
    REAL(dp), DIMENSION(n_bins_particle) :: speed_p
    REAL(dp), DIMENSION(n_bins_particle) :: vapmeanfp,FS_corr,D_vap_eff
...
SUBROUTINE UPDATE_MOLECULAR_DIFF_AND_CSPEED(vapour_prop)
    ...
    INTEGER                             :: sa           ! short for sulfuric acid index

    ! --- Molecule thermal speed (m/s) ALL MOLECULES -------------------------------------------------------------------
    vapour_prop%c_speed = SQRT(8D0*Kb*GTEMPK/(pi*vapour_prop%molec_mass))

    ! --- Molecular diffusivity, 2 methods, where Fullers is the default -----------------------------------
    ! --- (H2SO4 can be RH corrected) ----------------------------------------------------------------------
    IF (Use_atoms) THEN
        vapour_prop%diff  = 1D-7 * GTempK**1.75D0 * SQRT( 1/(Mair*1D3) + 1/(1d3*VAPOUR_PROP%molar_mass) )   &
                            / (GPres/1.01325D5 * (VAPOUR_PROP%diff_vol**(1D0/3D0) + 20.1**(1D0/3D0))**2D0)
    ELSE
        vapour_prop%diff  = 5d0/(16d0*Na* vapour_prop%molec_dia**2d0*(Mair * GPRES/(Rg*GTEMPK)))            &
                            * sqrt(Rg*GTEMPK*Mair/(2.*pi)*((VAPOUR_PROP%molar_mass+Mair)/VAPOUR_PROP%molar_mass))
    END IF


    ! --- Molecular diffusivity, sulfuric acid diffusivity is RH dependant ---------------------------------------------
    sa = vapour_prop%ind_H2SO4
    ! vapour_prop%diff(sa)  = 5d0/(16d0*Na* vapour_prop%molec_dia(sa)**2d0*(Mair * GPRES/(Rg*GTEMPK))) &
    !                         * sqrt(Rg*GTEMPK*Mair/(2.*pi)*((VAPOUR_PROP%molar_mass(sa)+Mair)/VAPOUR_PROP%molar_mass(sa)))
    !
