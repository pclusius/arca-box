...
print("Let's try to copile the model, have patience:")
out = os.system("make")

print()
if out != 0:
    print("""Compiling failed. First check that Fortran compiler is working. The default compiler is gfortran, if some other
compiler is used, change the variable "F90" in the "makefile" to corresponding compiler. If the compiler is ok, make sure that
netcdf-fortran and curl-dev is installed from Cygwin. Email the developers if you keep having troubles.
""")
else:
    print("+------------------------------------------+\n")
