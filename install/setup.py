...
if out == 0:
    ...
    f.write('%s'%le)
    f.write('while getopts o:i:s:a: flag%s'%le)
    f.write('do%s'%le)
    f.write('    case "${flag}" in%s'%le)
    f.write('        o) organic_compoundlist="--names ${OPTARG}";;%s'%le)
    f.write('        i) inorganic_compoundlist="-i ${OPTARG}";;%s'%le)
    f.write('        s) scaling="--scaling_${OPTARG}";;%s'%le)
    f.write('        a) scaleall="--scaleall_${OPTARG}";;%s'%le)
    f.write('    esac%s'%le)
    f.write('done%s'%le)
