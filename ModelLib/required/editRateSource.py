...
with open(os.path.join(f,'second_Rates.f90'), 'r') as file:
    for line in file:
        if re.search("RCONST\s*=\s*RCONST\s*\\*\s*R_F",line.strip('\n').upper()):
            hits +=1
        if re.search('^\s*END\s*SUBROUTINE\s*UPDATE_RCONST', line.upper()) and hits == 0:
...
with open(os.path.join(f,'second_Main.f90'), 'r') as file:
    for line in file:
        if re.search("RCONST\s*=\s*RCONST\s*\\*\s*R_F",line.strip('\n').upper()):
            hits +=1
        elif re.search("REAL\(DP\),\s*SAVE\s*::\s*R_F\(NREACT\)",line.strip('\n').upper()):
            hits +=1
        elif re.search("\s*!\s*MODIFY\s*RATE\s*CONSTANTS",line.strip('\n').upper()):
            hits +=1
        elif re.search('SUBROUTINE KPP_SETUP\(\)', line.strip('\n').upper()) != None:
...
with open(os.path.join(f,'second_Global.f90'), 'r') as file:
    for line in file:
        if re.search("REAL\(DP\),\s*SAVE\s*::\s*R_F\(NREACT\)\s*=\s*1D0",line.strip('\n').upper()):
            hits +=1

        if re.search('!\s*INLINED\s*GLOBAL\s*VARIABLE\s*DECLARATIONS', line.upper()) and hits == 0 and search_on:
