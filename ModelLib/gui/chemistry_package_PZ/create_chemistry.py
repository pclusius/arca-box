...
if __name__ == '__main__':
  ...
  # DEFFIX list injected by the user
  defFixOverrides = [s.strip() for s in args.defFixOverrides]  # strip the white spaces
  ...
  for str_k in ['KNO', 'KHO2', 'KRO2', 'KNO3', 'KTR', 'K16ISOM']:
    line_number_k, i1, i2 = index_containing_substring(file_lines, r'\b' + str_k + r'\b' + r'\s*=', False)
    if line_number_k >= 0:
  ...
  species_list = []
  deffix_print_list = []
  if extraTwist and len(defFixOverrides)>0:
    species_list = defFixOverrides
    deffix_print_list = (' = IGNORE ;\n,'.join(defFixOverrides)+' = IGNORE ;\n').split(',')
  ...
  if extraTwist:
      # removing possible DEFVARS that are DEFFIXed
      logging.info('============================================================')
      logging.info('')
      st_varFrom,_,_ = index_containing_substring(file_lines, '#DEFVAR')
      st_fixFrom,_,_ = index_containing_substring(file_lines, '#DEFFIX')
      if st_varFrom>st_fixFrom:
        for dfl in deffix_print_list:
          if '=' in dfl and dfl in file_lines[st_varFrom:]:
            i = file_lines[st_varFrom:].index(dfl)
            logging.info('Removing {0} from DEFVAR (already in DEFFIX)'.format(file_lines.pop(st_varFrom+i).strip()))
      else:
        for dfl in deffix_print_list:
          if '=' in dfl and dfl in file_lines[:st_fixFrom]:
            i = file_lines[:st_fixFrom].index(dfl)
            logging.info('Removing {0} from DEFVAR (already in DEFFIX)'.format(file_lines.pop(i).strip()))
      logging.info('')
      logging.info('============================================================')
